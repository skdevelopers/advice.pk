    ...
    <div class="container-fluid relative px-3">
        <div class="layout-specing">
            ...
            <div class="md:flex justify-between items-center mb-6">
                <h5 class="text-lg font-semibold">Edit Blog</h5>
            ...
            <div class="rounded-md shadow-sm bg-white p-6"
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <template x-if="!previewUrl && '{{ $blog->getFirstMediaUrl("blogs","webp") }}'">
                            <img src="{{ $blog->getFirstMediaUrl('blogs','webp') }}"
                                 class="mt-2 w-full h-auto rounded shadow" />
                        </template>
                    </div>
                    <div class="flex space-x-2">
                        <button type="button" @click="generateSeo()"
                                class="px-4 py-2 bg-blue-600 text-white rounded">Refresh AI SEO</button>
                    </div>
                    <div>
                    ...
                    <div>
                        <button @click="submit()" class="px-6 py-2 bg-green-600 text-white rounded">Update Blog</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <script>
            function blogForm(data) {
                return {
                    ...
                    async generateSeo() {
                        try {
                            let res = await axios.post(`/admin/blogs/${data.id}`, {
                                ...this.form,
                                prompt: 'seo',
                                _method: 'PUT'
                            });
                            ...
                            showToast('SEO refreshed', 'success');
                        } catch {
                            showToast('SEO failed', 'error');
                        }
                    },
                    async submit() {
                        ...
                        try {
                            await axios.post(`/admin/blogs/${data.id}`, formData, {
